...
func GetObservers() map[string]Observer {
	return defaultClient.GetObservers()
}

// On register configuration callback when the key's value is changed
func On(namespaceWithSuffix, key string, cb Observer) {
	defaultClient.On(namespaceWithSuffix, key, cb)
}
