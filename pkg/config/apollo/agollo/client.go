...
// On 注册配置变更回调函数
func (c *Client) On(namespaceWithSuffix string, key string, cb Observer) {
	c.observerDelegate.add(namespaceWithSuffix, key, cb)
}
...
func (c *Client) handleNamespaceUpdate(namespace string) error {
	...
	observers := c.observerDelegate.getObservers()
	suffix := path.Ext(change.Namespace)
	suffix = strings.TrimLeft(suffix, ".")
	switch suffix {
	case yamlSuffix:
		fallthrough
	case ymlSuffix:
		ProcessYamlDiffUnderCertainKeys(observers, change.Namespace, change.Changes[yamlChangeKey].OldValue, change.Changes[yamlChangeKey].NewValue)
	default:
		for k, c := range change.Changes {
			keyInObserverMap := fmt.Sprintf("%s%s%s", change.Namespace, namespaceToKeySep, k)
			// fmt.Printf("k = %s and keyInObserverMap = %s\n", k, keyInObserverMap)
			if o, ok := observers[keyInObserverMap]; ok {
				go o(k, c.OldValue, c.NewValue)
			}
		}
	}
...
// GetObservers return all registered cb
func (c *Client) GetObservers() map[string]Observer {
